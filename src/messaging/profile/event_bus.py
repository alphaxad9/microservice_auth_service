...
from src.domain.profile.events import ProfileEvent
...
AsyncEventHandlerFunction = Callable[[ProfileEvent], Awaitable[None]]

@dataclass(frozen=True)
class RegisteredHandler:
    handler: Union[BaseEventHandler, AsyncEventHandlerFunction]
    event_type: Type[ProfileEvent]


class ProfileEventBus:
    def __init__(self) -> None:
        self._handlers: Dict[Type[ProfileEvent], List[RegisteredHandler]] = {}
        self._lock = RLock()
    ...
    async def publish(self, event: ProfileEvent) -> None:
    ...
    def get_subscribed_events(self) -> List[Type[ProfileEvent]]:
        with self._lock:
            return list(self._handlers.keys())
        
profilebus = ProfileEventBus()
